...
def predict(model):
    periods = 365
    return model.predict(model.make_future_dataframe(periods=365)[-periods:])[['ds', 'yhat']].rename(columns = {'ds':'Date', 'yhat':'Close_Prediction'})
...
def get_stock_data(ticker, start, end):
    ticker_data = yf.download(ticker, start, end).reset_index(inplace=True)  # downloading the stock data from START to TODAY
    ticker_data['Date'] = ticker_data['Date'].dt.tz_localize(None)
    return ticker_data

tickers = {'MSFT':get_stock_data('MSFT', '2010-01-01', '2020-01-01'),
           'AAPL':get_stock_data('AAPL', '2010-01-01', '2020-01-01'),
           'GOOG':get_stock_data('GOOG', '2010-01-01', '2020-01-01')}
...
for stock_name in tickers.tickers:
